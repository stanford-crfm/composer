...
class DownstreamTaskEvaluator(ABC):
    ...
    def config_cartesian(config):
        ...
        for k in config_copy:
            if type(config_copy[k]) != list:
                config_copy[k] = list[config_copy[k]]
        return [dict(zip(config.keys(), value_combo)) for value_combo in itertools.product(*config_copy.values())]


    def task_name(self, config) -> str:
        options_label = config_to_str(config=config,delimiter="_",k_prefix="",v_prefix="=")
    ...
    def build_command(self, executables: list, defaults: dict, config: dict):
        ...
        for executable in executables:
            ...
            executable_config.update(config)
            sub_cmds.append(DownstreamTaskEvaluator.config_to_str(config=executable_config,delimiter=" ",k_prefix="--",v_prefix=" "))
        ...
        with htrack_block(f"Evaluating {self.artifact.name} of {self.run.name}"):
            ...
            try:
                ...
                if model_type_config:
                    cmd_config_combos = DownstreamTaskEvaluator.config_cartesian(self.hparams[model_type_config])
                    for cmd_config in cmd_config_combos:
                        hparam_cmd = self.command(config=hparam_config)
                        if self.environment:
                            ...
                            assert os.path.exists(conda_shell), f"Error: {conda_shell} does not exist"
                            # add conda set up to task command
                            setup_env_cmd = ". {conda_shell} ; conda activate {self.environment}"
