...
"""
Convert DeepSpeed-format checkpoint to Torch-format checkpoint.
...
def convert_checkpoint(deepspeed_checkpoint_path: str, output_dir: str) -> str:
    ...
    """
    model_checkpoint = torch.load(deepspeed_checkpoint_path, map_location=torch.device('cpu'))
...
def main():
    ...
    parser.add_argument(
        "--input-path", help="Path to DeepSpeed checkpoint"
    )
    parser.add_argument(
        "--output-dir", help="Path to output Torch checkpoint", default="."
    )
