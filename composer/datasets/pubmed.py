...
__all__ = ["PubmedDataset", "PubMedDatasetHparams"]
...
class PubMedDatasetHparams(DatasetHparams):
    ...
    Parameters:
        ...
        shuffle (bool): Whether to shuffle the samples in the dataset. Currently, shards are assigned and consumed
                        with deterministic per-device shard order, but shuffling affects the order of samples via
                        (per-device) shuffle buffers. (Default: `False`)
    ...
    max_samples: int = hp.optional(
        "Max number of post-processed token samples, used to set epoch size of the IterableDataset.",
        default=None,
    )
    ...
    max_seq_len: int = hp.optional(
        "The max sequence length of each token sample.", default=None
    )
    group_method: str = hp.optional(
        "How to group text samples into token samples. Either `truncate` or `concat`.",
        default=None,
    )
    mlm: bool = hp.optional(
        "Whether or not to use masked language modeling.", default=False
    )
    mlm_probability: float = hp.optional(
        "If `mlm=True`, the probability that tokens are masked.", default=0.15
    )
    ...
    def initialize_object(
        ...
        collate_fn = transformers.DataCollatorForLanguageModeling(
            tokenizer=pubmed_dataset.tokenizer,
            mlm=self.mlm,
...
class PubmedDataset(IterableDataset):
    """Builds a streaming, sharded, sized IterableDataset for the Pubmed dataset. Used for
    pretraining autoregressive or masked language models. Text samples are streamed directly from the cloud using
    HuggingFace's streaming backend. The text samples are then shuffled, tokenized, and grouped on-the-fly.
    ...
    def __init__(
        ...
        if self.max_samples % self.world_size != 0:
            ...
            log.warning(
                f"Max samples will be truncated from {max_samples}->{new_max_samples} to maintain divisibility "
        ...
        if (
            self.max_samples > original_approx_samples
            and self.group_method == "truncate"
        ):
            raise ValueError(
                f"Max samples was set to {self.max_samples} with group_method 'truncate' but split '{split}' "
                f"has only {original_approx_samples}.)"
            )
        if self.group_method == "concat":
            log.warning(
                f"When using group_method 'concat', sequential token samples are concatenated and chunked into "
                f"fixed-length samples of size max_seq_len={self.max_seq_len}. "
        ...
        if not isinstance(text_dataset, datasets.IterableDataset):
            raise ValueError("Unable to build sharded PubMed Dataset.")
    ...
    # Shards are split per-device, e.g. For 8 shards and 4 devices... device0 receives shards [0, 4],
    # device1 receives shards [1, 5].. etc.
    # If there are not enough shards for devices (common with small validation splits), then shards are sent
    # to multiple devices but subsampled internally.
    # E.g. For 2 shards and 4 devices... device0 receives shards [0] and consumes samples 0, 2, 4, ... device1
    # receives shards [0] and consumes samples 1, 3, 5, ... etc.
    # Currently, either (num_shards % num_devices == 0) or (num_devices % num_shards == 0) is enforced for
    # efficient streaming, but this could be relaxed in the future at the cost of increased bandwidth
    # (have many more devices read the same shards and subsample)
    def _shard_dataset(self, dataset):
        ...
        if self.num_shards != len(filepaths):
            raise ValueError(
                f"Found {len(filepaths)} shards, expected {self.num_shards}"
            )
        ...
        if self.num_shards < self.world_size:
            log.warning(
                f"Not enough unique shards ({self.num_shards}) for world size ({self.world_size}). "
                f"Splitting shards among devices."
            )
            if self.world_size % self.num_shards != 0:
                raise ValueError(
                    f"Cannot evenly split {self.num_shards} shards among {self.world_size} devices"
                )
    ...
    # Prepare a batch of token samples for pretraining, by grouping them into fixed-length token samples, with either
    # 'truncate' or 'concat' group methods.
    # If using 'truncate', each token sample is padded/truncated to 'self.max_seq_len', and there is no mixing between
    # adjacent token samples.
    # Using 'truncate' may be computationally inefficient if 'self.max_seq_len' is large, as the suffix of each token
    # sample will consist of empty padding.
    # Using 'truncate' may also be data inefficient as it will discard the suffix of any token sample that is larger
    # than 'self.max_seq_len'.
    # If using 'concat', the batch of token samples is concatenated and chunked, such that every new token sample is
    # exactly 'self.max_seq_len' long with no padding.
    # Using 'concat' may drop a small amount of data at the end of each batch if the total number of tokens is not
    # divisible by 'self.max_seq_len'.
    # Using 'concat' will alter the number of token samples in the iterable dataset, and differently per-device,
    # so we require the user to provide a 'self.max_samples' limit to ensure epoch-boundary synchronization
    # across devices.
    def _group_tokens(self, token_batch):
        if self.group_method == "truncate":
            # No processing needed, as 'self._tokenize()' has already padded / truncated each token sample
            # to 'self.max_seq_len'.
            return token_batch
            ...
            for k, v in token_batch.items():
                ...
                if num_tokens is None:
                    ...
                    raise ValueError(
                        "Not all values in concat_tokens dict have same len()"
                    )
                else:
                    pass
            if num_tokens is None:
